...
router.get('/course/:courseId', authMiddleware, roleCheck(['student']), asyncHandler(async (req, res) => {
...
router.get('/course/:courseId/resume', authMiddleware, roleCheck(['student']), asyncHandler(async (req, res) => {
  const lectures = await prisma.lecture.findMany({
    where: { courseId: parseInt(req.params.courseId) },
